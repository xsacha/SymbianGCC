...
typedef struct promote_info *promote_info_p;
DEF_VEC_P (promote_info_p);
DEF_VEC_ALLOC_P (promote_info_p,heap);

/* Helper function for qsort.  */

static int
compare_promote_info (const void *p1, const void *p2)
{
  const promote_info_p * const pi1 = (const promote_info_p *) p1;
  const promote_info_p * const pi2 = (const promote_info_p *) p2;

  return (*pi1)->uid - (*pi2)->uid;
}
...
/* Counter for things inserted into `phi_nodes_to_promote'.  */
...
static void
analyze_loop (struct promote_info *pi, bool *changed)
{
  /* We previously determined we can't promote this; stop.  */
...
/* Add PI->LOOP_INDEX_NAME to the set of variables, PSET, that we are
   considering for promotion.  */

static void
add_variable (const struct promote_info *pi, struct pointer_set_t *pset)
{
  if (pi->loop_index_name != NULL_TREE)
    {
      int presentp = pointer_set_insert (pset, pi->loop_index_name);
      gcc_assert (!presentp);
    }
}

/* Helper function to collect things from a pointer_set into a VEC.  */

static bool
record_promotable_variables (const void *pi_, void *data)
{
  const struct promote_info *pi = (const struct promote_info *) pi_;
  VEC(promote_info_p,heap) **pvec = (VEC(promote_info_p,heap) **) data;

  VEC_safe_push (promote_info_p, heap, *pvec,
		 CONST_CAST (struct promote_info *, pi));
...
static bool
create_promoted_variable (struct promote_info *pi)
{
  if (pi->can_be_promoted_p)
    {
...
rebuild_with_promotion (gimple stmt, struct promote_info *pi)
{
  ...
  switch (gimple_code (stmt))
	...
	gimple *phip;
	gsi = gsi_for_stmt (stmt);
	phip = gsi_stmt_ptr (&gsi);
	gimple_set_uid (*phip, phi_uid++);
	pointer_set_insert (phi_nodes_to_promote, phip);
      }
      break;
    default:
      gcc_unreachable ();
...
static bool
promote_variable (const struct promote_info *pi)
{
  if (pi->can_be_promoted_p)
    {
...
static bool
record_phi_node (const void *phi_, void *data)
{
  const gimple *phi = (const gimple *) phi_;
  VEC(gimple_p,heap) **gvec = (VEC(gimple_p,heap) **) data;

  VEC_safe_push (gimple_p, heap, *gvec, CONST_CAST (gimple *, phi));

  return true;
}

static void
promote_phi_node (const gimple *cphi)
{
...
static int
compare_gimple_phis (const void *p1, const void *p2)
{
  const gimple_p * const g1 = (const gimple_p *) p1;
  const gimple_p * const g2 = (const gimple_p *) p2;

  return gimple_uid (**g1) - gimple_uid (**g2);
}
...
promote_short_indices (void)
{
  ...
  /* Make sure that we traverse the elements in a predictable order.  */
  pvec = VEC_alloc (promote_info_p, heap, max_iterations);
  pointer_set_traverse (promotion_info, record_promotable_variables, &pvec);

  qsort (VEC_address (promote_info_p, pvec),
	 VEC_length (promote_info_p, pvec),
	 sizeof (promote_info_p), compare_promote_info);
  ...
  while (changed)
    {
      ...
      for (ix = 0; VEC_iterate (promote_info_p, pvec, ix, pi); ix++)
	add_variable (pi, promotable_names);
  ...
  if (n_promoted != 0)
    {
      ...
      for (ix = 0; VEC_iterate (promote_info_p, pvec, ix, pi); ix++)
	{
	  create_promoted_variable (pi);
	  promote_variable (pi);
	}

      /* Iterate over phi nodes, stably.  */
      pointer_set_traverse (phi_nodes_to_promote, record_phi_node, &gvec);

      qsort (VEC_address (gimple_p, gvec), VEC_length (gimple_p, gvec),
	     sizeof (gimple_p), compare_gimple_phis);

      for (ix = 0; VEC_iterate (gimple_p, gvec, ix, g); ix++)
	promote_phi_node (g);

      VEC_free (gimple_p, heap, gvec);
    }

  VEC_free (promote_info_p, heap, pvec);
